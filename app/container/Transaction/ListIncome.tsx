...
import { EProductTypes, EPaymentStatus, EPaymentTypes, TSelectItem } from "~/_types"
...
export default function ListIncomeContainer() {
    ...
    const { isLoading, data, refetch } = useGetIncomesQuery({
        merchant_id:account?.user?.merchant_id as number,
        ...filterData,
    })
    ...
    const onFilterCategory = useCallback((id:number, item:TSelectItem|undefined)=>{
        setFilterData((prev)=>({...prev, category_id:id==0?undefined:id}))
        setCategoryName(item?.label??"")
    },[filterData, setFilterData, categoryName, setCategoryName])
        ...
        <div className="pb-2">
            <Card className="max-w-full">
                ...
                <div className="flex flex-col justify-between">
                    <CategorySelector label={null} 
                        handleUpdate={onFilterCategory} 
                        merchantId={Number(account?.user?.merchant_id)}
                        defaultValue={filterData.category_id} />
                </div>

                {isAdmin() && <div className="flex flex-col justify-between">
                    <StaffSelector label={null} 
            ...
            <Card className="max-w-full mt-5">
                <div className="flex flex-col items-center justify-center">
                    ...
                    <h5 className="text-xl mb-1 leading-none text-gray-900 dark:text-white">
                        Total Pemasukan {categoryName !== "" && <span>({categoryName})</span>}
                    </h5>
                    <h3 className="text-green-500 font-bold text-2xl">
                        {currency(data?.transactions_debit?.total ?? 0)}
                    </h3>
                </div>

                {filterData.category_id === undefined && <div>
                    <div className="flex items-center justify-between border-t border-gray-700 pt-4">
                        <div>
                            <h5 className="text-lg mb-1 text-gray-900 dark:text-white">
                                Pemasukan <br /> Transfer
                            </h5>
                            <h3 className="text-green-500 mb-2 font-bold text-2xl">
                                {currency(data?.transfer?.total??0)}
                            </h3>
                        </div>

                        <div className="text-right">
                            <h5 className="text-lg mb-1 text-gray-900 dark:text-white">
                                Pemasukan <br /> Tarik Tunai
                            </h5>
                            <h3 className="text-green-500 font-bold mb-2 text-2xl">
                                {currency(data?.withdrawal?.profit??0)}
                            </h3>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between border-t border-gray-700 pt-4">
                        <div>
                            <h5 className="text-lg mb-1 text-gray-900 dark:text-white">
                                Pemasukan <br />Voucher
                            </h5>
                            <h3 className="text-green-500 mb-2 font-bold text-2xl">
                                {currency(data?.voucher?.total??0)}
                            </h3>
                        </div>

                        <div className="text-right">
                            <h5 className="text-lg mb-1 text-gray-900 dark:text-white">
                                Pemasukan <br />Pulsa/Paket Data
                            </h5>
                            <h3 className="text-green-500 font-bold mb-2 text-2xl">
                                {currency(data?.elektrik?.total??0)}
                            </h3>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between border-t border-gray-700 pt-4">
                        <div>
                            <h5 className="text-lg mb-1 text-gray-900 dark:text-white">
                                Pemasukan <br />SP Perdana
                            </h5>
                            <h3 className="text-green-500 mb-2 font-bold text-2xl">
                                {currency(data?.sp?.total??0)}
                            </h3>
                        </div>

                        <div className="text-right">
                            <h5 className="text-lg mb-1 text-gray-900 dark:text-white">
                                Pemasukan <br />Accessories
                            </h5>
                            <h3 className="text-green-500 font-bold mb-2 text-2xl">
                                {currency(data?.accessories?.total??0)}
                            </h3>
                        </div>
                    </div>
                </div>}
